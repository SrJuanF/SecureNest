 ...
 * @param customFolder Optional custom folder name (overrides deploymentType)
 * @returns The deployment configuration used
 */
export function saveDeploymentData(deploymentData: any, baseDir: string, isConverter: boolean = true, customFolder?: string) {
    const deploymentType = customFolder || (isConverter ? "converter" : "standalone");
    ...
    const fileName = customFolder 
        ? `${customFolder}-${timestamp}.json`
        : (isConverter ? `converter-${timestamp}.json` : `standalone-${timestamp}.json`);
    
    const latestFileName = customFolder 
        ? `latest-${customFolder}.json`
        : (isConverter ? "latest-converter.json" : "latest-standalone.json");
